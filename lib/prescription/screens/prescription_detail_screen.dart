...
class PrescriptionDetailScreen extends StatefulWidget {
  ...
  @override
  _PrescriptionDetailScreenState createState() => _PrescriptionDetailScreenState(this.idd,this.useridd,this.prescriptionid);
}
...
class _PrescriptionDetailScreenState extends State<PrescriptionDetailScreen> {
  ...
  Future<String> getMedicineByID(var medicineid , var ionid) async {
      ...
      var medicinedata = await http.get(Auth().linkURL +"api/getMedicineById?id=${medicineid}&ion_id="+ionid);
   ...
   Future<PrescriptionDetails> _responseFuture() async {
    ...
    var data = await http.get(Auth().linkURL +"api/viewPrescription?id=${prescriptionid}&user_ion_id="+useridd);
    ...
    var ini_user = jsondata["user"];
      ...
      var borntime = DateTime(int.parse(agearray[2] ) ,int.parse(agearray[1] ) , int.parse( agearray[0]) );
      ...
      if (ini_prescription["medicine"] != ""){
        ...
        for (var sorti in medicine_sortList){
          ...
          String medicinename = await getMedicineByID(temp_medicine_sortList[0], useridd);
          ...
          full_medicine_String = full_medicine_String + "${medicinename} - ${temp_medicine_sortList[1]} - ${temp_medicine_sortList[2]} - ${temp_medicine_sortList[3]} - ${temp_medicine_sortList[4]} \n";
      ...
      var historyz = ini_prescription["symptom"].replaceAll("<p>", "").replaceAll("</p>", "\n").replaceAll("<h1>", "").replaceAll("</h1>", "\n").replaceAll("<h2>", "").replaceAll("</h2>", "\n").replaceAll("<h3>", "").replaceAll("</h3>", "\n").replaceAll("<h4>", "").replaceAll("</h4>", "\n");

      var notez = ini_prescription["note"].replaceAll("<p>", "").replaceAll("</p>", "\n").replaceAll("<h1>", "").replaceAll("</h1>", "\n").replaceAll("<h2>", "").replaceAll("</h2>", "\n").replaceAll("<h3>", "").replaceAll("</h3>", "\n").replaceAll("<h4>", "").replaceAll("</h4>", "\n");
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizations.of(context).prescriptionDetail,
          style: TextStyle(
        ...
        onPressed:() => Navigator.of(context).pushReplacementNamed(UserPrescriptionsScreen.routeName),
              ...
              if (response.data == null) {
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text("${response.data.doctor_name}",
                      style: TextStyle(
                        fontSize: 20
                      ),
                    ),
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Container(
                          child: Text("${AppLocalizations.of(context).date}: ${response.data.date}"),
                        ),
                        SizedBox(width: 20,),

                        Container(
                          child: Text("${AppLocalizations.of(context).prescriptionId}: ${response.data.id}"),
                        ),
                      ],
                    ),
                  ) ,
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Container(
                          child: Text("${AppLocalizations.of(context).patient}: ${response.data.patient_name}"),
                        ),
                        SizedBox(width: 20,),

                        Container(
                          child: Text("${AppLocalizations.of(context).patientId}: ${response.data.patient_id}"),
                        ),
                      ],
                    ),
                  ) ,
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Container(
                          child: Text("${AppLocalizations.of(context).age}: ${response.data.age} "),
                        ),
                        SizedBox(width: 20,),

                        Container(
                          child: Text("${AppLocalizations.of(context).gender}: ${response.data.gender}"),
                        ),
                      ],
                    ),
                  ) ,
                  ...
                  Container(
                    width: MediaQuery.of(context).size.width*1,
                    padding: const EdgeInsets.only(left: 20, right:20),              
                    child: Text("${AppLocalizations.of(context).rx}",
                      style: TextStyle(
                        fontSize: 18,
                  ...
                  Container(
                    width: MediaQuery.of(context).size.width*1,
                    padding: const EdgeInsets.only(left: 20, right:20),              
                    child: Text("${response.data.medicine}",
                      style: TextStyle(
                        fontSize: 15,
                      ),
                    ),
                  ...
                  Container(
                    width: MediaQuery.of(context).size.width*1,
                    padding: const EdgeInsets.only(left: 20, right:20),              
                    child: Text("${response.data.symptom}",
                      style: TextStyle(
                        fontSize: 15,
                      ),
                    ),
                  ...
                  Container(
                    width: MediaQuery.of(context).size.width*1,
                    padding: const EdgeInsets.only(left: 20, right:20),              
                    child: Text(AppLocalizations.of(context).advice,
                      style: TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.bold,
                      ),
                  ...
                  Container(
                    width: MediaQuery.of(context).size.width*1,
                    padding: const EdgeInsets.only(left: 20, right:20),              
                    child: Text("${response.data.advice}",
                      style: TextStyle(
                        fontSize: 15,
                      ),
                    ),
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Container(child: Text("${response.data.hospital_title}",
                                style: TextStyle(
                                  fontSize: 20
                                ),
                              )),
                              Container(
                                child: Text(" ${response.data.hospital_address}",
                                  style: TextStyle(
                                    fontSize: 11
                                  ),
                                ),
                              ),
                              Container(child: Text("${response.data.hospital_phone}",
                                  style: TextStyle(
                                    fontSize: 12
                                  ),
                              )),
